...
podman run -d --name schema -h ${DOMAIN} -e "DOMAIN=${DOMAIN}" -v $(pwd)/src/:/opt/app-root/src/:z -p 443:8443 -p 80:8080 schema
curl --head -X GET --retry 10 --retry-connrefused --retry-delay 2 --fail http://localhost:80 || (echo "Server did not start"; exit 1;) 
